class FireController < ApplicationController
  ...
  def all_known
    ...
    @sign_symbol = @sign.to_sym
    @zodiacs = Zodiac.list
    @this_zodiac = @zodiacs.fetch(@sign_symbol)
    @horoscope = @this_zodiac.fetch(:horoscope)

    @lucky = Array.new

    5.times do
      another_one = rand(1...100)
      @lucky.push(another_one)
    end

    render({:template => "flame_templates/all_known.html.erb"})
  end
